...
def analyze_and_plot_comprehensive_metrics(original_log, deduplicated_log, perf_data):
    ...
    total_valid_crashes = sum(1 for e in deduplicated_log if e.get('is_physical_crash', False))
    ...
    # Use the deduplicated log as the time horizon for downstream metrics.
...
def main():
    ...
    if max_depth > 0:
        ...
        print(f"Filtered raw log to fuzz-stage candidates before dedup: {len(original_log_data)} entries.")
    else:
        # 如果最大深度只有0，说明是纯随机测试 (RT)，保留所有数据
        print("Detected Random Testing (RT) data (Max Depth == 0). Keeping all candidates before dedup.")
    ...
    if not deduplicated_log: 
        ...
        return
    print(f"Deduplicated test cases retained for analysis: {len(deduplicated_log)}")
