...
def run_tsne(data, n_samples):
    ...
    tsne = TSNE(
        ...
        max_iter=1000, # 使用 'max_iter' 替换 'n_iter'
        random_state=42
    )
...
# --- 5. 图表3：崩溃代数直方图 (基于去重数据) ---

def build_parent_map(original_log_data):
    """
    遍历 *原始* 日志，构建一个 {seed_id: parent_id} 的完整家谱图。
    """
    parent_map = {}
    for entry in original_log_data:
        seed_id = entry.get('seed_id')
        parent_id = entry.get('parent_id')
        if seed_id is not None and seed_id not in parent_map:
             parent_map[seed_id] = parent_id
    print(f"家谱图构建完成，总共找到 {len(parent_map)} 个独特的父种子。")
    return parent_map

def get_generation(seed_id, parent_map, generation_cache):
    """
    ...
    try:
        parent_generation = get_generation(parent_id, parent_map, generation_cache)
        ...
        print(f"警告: 追溯 seed_id {seed_id} (parent: {parent_id}) 时出错: {e}")
        return -1 # 错误代码

def plot_generation_histogram(deduplicated_log, parent_map):
    """
    绘制 *独特崩溃输入* 的代数直方图。
    """
    print(f"\n[图表 3] 正在分析崩溃代数...")
    
    generation_cache = {} # 为此次分析创建缓存
    crash_generations = []
    ...
    for entry in deduplicated_log:
        if entry.get('did_crash', False):
            ...
            parent_generation = get_generation(parent_seed_id, parent_map, generation_cache)
...
def main():
    ...
    # 5. 运行图表 3
    # 注意：家谱图必须基于 *原始* 日志构建，才能追溯所有血统
    parent_map = build_parent_map(original_log_data)
    if parent_map:
        plot_generation_histogram(deduplicated_log, parent_map)
