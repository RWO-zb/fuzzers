...
POSTHOC_JUDGEMENT_CANDIDATES = 'random1022/posthoc_carl_replay_input_judgements.json',
...
def resolve_posthoc_judgement_path(candidates):
    for path in candidates:
        if os.path.exists(path):
            return path
    print(f"Warning: no posthoc judgement file found in candidates: {candidates}")
    return None

def load_incompletable_keys(candidates):
    path = resolve_posthoc_judgement_path(candidates)
    if path is None:
        return set()
...
def load_data(csv_path):
    if not os.path.exists(csv_path):
        ...
        return None
    try:
        incompletable_keys = load_incompletable_keys(POSTHOC_JUDGEMENT_CANDIDATES)
        df = pd.read_csv(csv_path)
        if 'phase' in df.columns:
            if 'Phase2' in df['phase'].values:
                df = df[df['phase'] == 'Phase2']
            elif 'RT' in df['phase'].values:
                df = df[df['phase'] == 'RT']

        time_col = 'global_time' if 'global_time' in df.columns else 'elapsed_time'
        input_col = 'current_input' if 'current_input' in df.columns else 'input_post'
        generation_col = 'generation' if 'generation' in df.columns else 'mutation_generation'
        ...
        for source_index, row in df.iterrows():
            ...
            raw_time = row.get(time_col, 0.0)
            raw_generation = row.get(generation_col, 0)
            entry = {
                'task_id': task_id,
                ...
                'crash_time': float(raw_time) if pd.notna(raw_time) else 0.0,
                'mutate_state_str': row.get(input_col, 'None'),
                'parent_depth': int(float(raw_generation)) if pd.notna(raw_generation) else 0,
